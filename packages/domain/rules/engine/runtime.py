...
class RuleExecutionEngine:
    ...
    def execute_rules(self, ruleset: CompiledRuleSet, data: pd.DataFrame) -> ExecutionResult:
        """
        ...
        try:
            ...
            for phase in plan.phases:
                ...
                phase_result = self._execute_phase(phase=phase, rules=ruleset.rules, data=data)
    ...
    def _execute_phase(
        self, phase, rules: dict[str, CompiledRule], data: pd.DataFrame  # ExecutionPhase
    ) -> ExecutionResult:
        """Executa uma fase específica do plano."""
        phase_result = ExecutionResult()

        for group in phase.rule_groups:
            group_result = self._execute_rule_group(group=group, rules=rules, data=data)
            self._merge_results(phase_result, group_result)

        return phase_result

    def _execute_rule_group(
        self, group: RuleGroup, rules: dict[str, CompiledRule], data: pd.DataFrame
    ) -> ExecutionResult:
    ...
    def _execute_vectorized(self, rules: list[CompiledRule], data: pd.DataFrame) -> ExecutionResult:
        ...
        for rule in rules:
            try:
                ...
                if rule.type == ActionType.ASSERT:
                    ...
                    violations = self._create_violations_from_mask(rule, ~condition_mask, data)
                    ...
                    transformations = self._apply_transform_vectorized(rule, condition_mask, data)
                    for transform in transformations:
                        result.add_transformation(transform)

                elif rule.type == ActionType.SUGGEST:
                    # Sugestões vetorizadas
                    suggestions = self._generate_suggestions_vectorized(rule, condition_mask, data)
    ...
    def _execute_parallel(self, rules: list[CompiledRule], data: pd.DataFrame) -> ExecutionResult:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit tasks
            future_to_rule = {
                executor.submit(self._execute_single_rule, rule, data): rule for rule in rules
            }
    ...
    def _execute_sequential(self, rules: list[CompiledRule], data: pd.DataFrame) -> ExecutionResult:
    ...
    def _execute_single_rule(self, rule: CompiledRule, data: pd.DataFrame) -> ExecutionResult:
    ...
    def _execute_global_rule(self, rule: CompiledRule, data: pd.DataFrame) -> ExecutionResult:
    ...
    def _execute_column_rule(self, rule: CompiledRule, data: pd.DataFrame) -> ExecutionResult:
    ...
    def _execute_row_rule(
        self, rule: CompiledRule, row: pd.Series, row_index: int
    ) -> ExecutionResult:
    ...
    def _evaluate_condition_vectorized(
        self, condition: CompiledCondition, data: pd.DataFrame
    ) -> pd.Series:
        """Avalia condição de forma vetorizada."""
    ...
    def _evaluate_simple_condition_vectorized(
        self, condition: CompiledCondition, data: pd.DataFrame
    ) -> pd.Series:
        ...
        if operator == "eq":
            ...
            if condition.case_sensitive:
                ...
                return (
                    data[condition.field].str.lower().str.startswith(str(value).lower(), na=False)
                )
            ...
            return data[condition.field].isna() | (
                data[condition.field].astype(str).str.strip() == ""
            )
        elif operator == "not_empty":
            return ~(
                data[condition.field].isna() | (data[condition.field].astype(str).str.strip() == "")
            )
    ...
    def _evaluate_logical_condition_vectorized(
        self, condition: CompiledCondition, data: pd.DataFrame
    ) -> pd.Series:
    ...
    def _evaluate_row_condition(self, condition, field_value, row):
        ...
        operator = (
            ...
            if hasattr(condition.operator, "lower")
            else str(condition.operator).lower()
        )
        value = (
            condition.compiled_value
            if hasattr(condition, "compiled_value") and condition.compiled_value is not None
        ...
        if operator == "eq":
            ...
            pattern = (
                condition.compiled_value
                if hasattr(condition, "compiled_value") and condition.compiled_value
                else value
            )
            try:
