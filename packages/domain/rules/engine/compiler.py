...
class CompilationError(Exception):
    ...
    def __init__(
        self, message: str, rule_id: str | None = None, line_number: int | None = None
    ):
...
class RuleCompiler:
    ...
    def compile_yaml(self, yaml_content: str | dict) -> CompiledRuleSet:
        """
        ...
        try:
            ...
            compatibility_config = self._create_compatibility_config(
                yaml_data.get("compatibility", {})
            )
    ...
    def _compile_ccm_mapping(self, mapping_data: dict) -> CCMMapping:
        ...
        for ccm_field, mapping_config in mapping_data.items():
            if isinstance(mapping_config, str):
                ...
                field_mappings[ccm_field] = FieldMapping(
                    source_field=mapping_config, required=False
                )
        ...
        return CCMMapping(
            field_mappings=field_mappings, transforms=transforms, validation_order=validation_order
        )
    ...
    def _compile_single_rule(self, rule_data: dict) -> CompiledRule:
        ...
        if "condition" in rule_data:
            ...
            condition = CompiledCondition(
                type=ConditionType.SIMPLE, operator="not_empty", field=field
            )
    ...
    def _compile_simple_condition(
        self, condition_data: dict, default_field: str
    ) -> CompiledCondition:
    ...
    def _compile_logical_condition(
        self, condition_data: dict, default_field: str
    ) -> CompiledCondition:
        """Compila condição lógica."""
        if "and" in condition_data:
            subconditions = [
                self._compile_condition(subcond, default_field) for subcond in condition_data["and"]
            ]
            return CompiledCondition(
            ...
            subconditions = [
                self._compile_condition(subcond, default_field) for subcond in condition_data["or"]
            ]
            return CompiledCondition(
                type=ConditionType.LOGICAL,
                operator="or",
    ...
    def _compile_action(self, action_data: dict, rule_type: ActionType) -> CompiledAction:
        ...
        if action_type == ActionType.ASSERT:
            return CompiledAction(
                type=action_type, stop_on_error=action_data.get("stop_on_error", False)
            )
            ...
            return CompiledAction(type=action_type, suggestions=suggestions, confidence=confidence)
        else:
    ...
    def _create_rule_groups(
        self, rules: dict[str, CompiledRule], dependencies: dict[str, set[str]]
    ) -> list[RuleGroup]:
        ...
        for rule in sorted_rules:
            ...
            if not current_group or (
                ...
                and self._can_group_rules(current_group, [rule.id], dependencies)
            ):
    ...
    def _can_group_rules(
        self, current_group: list[str], new_rules: list[str], dependencies: dict[str, set[str]]
    ) -> bool:
    ...
    def _get_group_dependencies(
        self, rule_ids: list[str], dependencies: dict[str, set[str]]
    ) -> list[str]:
        ...
        if validation_groups:
            phases.append(
                ExecutionPhase(
                    ...
                    can_vectorize=any(
                        g.execution_mode == ExecutionMode.VECTORIZED for g in validation_groups
                    ),
                )
            )
        ...
        if transform_groups:
            phases.append(
                ExecutionPhase(
                    ...
                    can_vectorize=any(
                        g.execution_mode == ExecutionMode.VECTORIZED for g in transform_groups
                    ),
                )
            )
