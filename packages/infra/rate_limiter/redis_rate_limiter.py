...
from src.infrastructure.logging.utilities import LoggingPort, log_rate_limit_check
...
class InMemoryRateLimiter(RateLimiter, LoggingPort):
    ...
    def set_custom_limit(self, tenant_id: str, limit: int, resource: str | None = None) -> None:
        """
    ...
    def get_usage(self, tenant_id: TenantId, resource: str) -> dict[str, Any]:
        ...
        usage = {
            ...
            "denied_requests": bucket["denied_requests"],
            "denial_rate": (
                round((bucket["denied_requests"] / bucket["total_requests"]) * 100, 2)
                if bucket["total_requests"] > 0
                else 0.0
            ),
        }

        self._logger.info("rate_limit_usage_query", **usage, correlation_id=get_correlation_id())
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "overall_denial_rate": (
                round((total_denied / total_requests) * 100, 2) if total_requests > 0 else 0.0
            ),
        ...
        self._logger.info("rate_limiter_stats", **stats, correlation_id=get_correlation_id())
