...
from shared.logging.security import AuditEventType, AuditLogger, SecurityEventType, SecurityLogger
...
class ConcreteLogPublisher(LogPublisher):
    ...
    def _publish_value_object_validation_event(self, event: ValueObjectValidationEvent) -> None:
        ...
        if event.validation_result == "failed":
            ...
            log_data.update({"error_type": event.error_type, "error_reason": event.error_reason})
            self._logger.warning(f"{event.value_object_type.lower()}_validation_failed", **log_data)
        else:
            # Log successful validations as debug events
            self._logger.debug(f"{event.value_object_type.lower()}_created", **log_data)
