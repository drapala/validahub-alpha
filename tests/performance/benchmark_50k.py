...
from src.domain.rules.engine.ccm import CCM
...
class PerformanceBenchmark:
    ...
    def run_benchmark(self) -> BenchmarkReport:
        ...
        logger.info(f"SLO Target: {self.config.max_execution_time_ms}ms, {self.config.max_memory_mb}MB")
        ...
        try:
            ...
            for iteration in range(self.config.iterations):
                ...
                logger.info(f"Iteração {iteration + 1}: {iteration_metrics.execution_time_ms:.2f}ms, "
    ...
    def _generate_product_title(self) -> str:
        ...
        products = [
            ...
            'TV Smart LG', 'Geladeira Electrolux', 'Máquina de Lavar Brastemp',
            'Air Fryer Philco', 'Aspirador Electrolux', 'Micro-ondas Panasonic',
            'Tênis Nike', 'Relógio Casio', 'Perfume Natura'
        ]
        
        variants = [
            '128GB Preto', '256GB Azul', '15 Polegadas', '55 Polegadas',
            '462L Frost Free', '12kg Automática', '3L Inox', '1200W',
            '30L Digital', 'Air Max Masculino', 'Digital Resistente',
            'Essencial 100ml'
        ]
    ...
    def _generate_brand(self) -> str:
        ...
        brands = [
            'Samsung', 'Apple', 'LG', 'Sony', 'Electrolux', 'Brastemp',
            'Philco', 'Panasonic', 'Nike', 'Adidas', 'Casio', 'Natura'
        ]
    ...
    def _log_summary(self, report: BenchmarkReport):
        ...
        logger.info("=" * 60)
        logger.info(f"SLO Compliance: {report.slo_compliance_rate:.1%} ({'PASS' if report.passes_slo else 'FAIL'})")
        logger.info(f"Avg Time: {report.avg_execution_time_ms:.1f}ms (target: {report.config.max_execution_time_ms:.0f}ms)")
        logger.info(f"Avg Throughput: {report.avg_throughput_rps:.0f} rps (target: {report.config.min_throughput_rps:.0f} rps)")
        logger.info(f"Peak Memory: {report.peak_memory_mb:.1f}MB (limit: {report.config.max_memory_mb:.0f}MB)")
