...
class TestAutomaticDeletionAfterRetentionPeriod:
    ...
    def test_schedule_automatic_deletion__when_retention_period_defined__creates_deletion_schedule(
        ...
        deletion_candidates = [
            {
                "data_id": "job_001",
                "created_at": datetime.now(UTC) - timedelta(days=730),  # 2+ years old
                "retention_expires_at": datetime.now(UTC) - timedelta(days=1),  # Expired
                "data_category": DataCategoryEnum.JOB_DATA.value,
            },
            {
                "data_id": "job_002",
                "created_at": datetime.now(UTC) - timedelta(days=750),  # 2+ years old
                "retention_expires_at": datetime.now(UTC) - timedelta(days=20),  # Expired
                "data_category": DataCategoryEnum.JOB_DATA.value,
            },
        ]
        ...
        use_case = ScheduleAutomaticDeletionUseCase(
            retention_repo=mock_retention_repo, audit_log_port=mock_audit_log_port
        )
        ...
        mock_retention_repo.find_expired_data.assert_called_once_with(
            tenant_id=tenant_id, data_category=DataCategoryEnum.JOB_DATA.value
        )
...
class TestCascadeDeletionInAllSystems:
    ...
    def test_cascade_deletion__when_user_data_deleted__removes_from_all_related_systems(
        ...
        related_data_locations = {
            "primary_database": ["users.user_id", "jobs.user_id", "files.uploaded_by"],
            "backup_database": ["backup_users.user_id", "backup_jobs.user_id"],
            "cache_systems": [
                f"user_profile:{user_id}",
                f"user_jobs:{user_id}",
                f"user_files:{user_id}",
            ],
            "log_files": ["access.log", "audit.log", "job_processing.log"],
            "file_storage": [
                f"uploads/{tenant_id}/{user_id}/",
                f"processed/{tenant_id}/{user_id}/",
            ],
        }
        ...
        # Act
        result = use_case.execute(
            tenant_id=tenant_id, user_id=user_id, deletion_reason="retention_period_expired"
        )
...
class TestSoftDeleteWithAnonymization:
    ...
    def test_soft_delete_with_anonymization__when_data_expired__anonymizes_immediately(
        self, mock_retention_repo: Mock, mock_audit_log_port: Mock, tenant_id: str
    ):
        """
...
class TestBackupDataAlsoDeleted:
    ...
    def test_delete_backup_data__when_retention_expires__removes_from_all_backup_locations(
        ...
        backup_locations = [
            {
                ...
                "backup_date": datetime(2024, 1, 15, 3, 0, 0, tzinfo=UTC),
            },
            {
                "backup_id": "weekly_backup_2024_w03",
                "location": "s3://backups/weekly/2024-w03/",
                "contains_data": ["user_profiles", "job_history"],
                "backup_date": datetime(2024, 1, 21, 3, 0, 0, tzinfo=UTC),
            },
            {
                "backup_id": "monthly_backup_2024_01",
                "location": "glacier://backups/monthly/2024-01/",
                "contains_data": ["user_profiles", "job_history"],
                "backup_date": datetime(2024, 1, 31, 3, 0, 0, tzinfo=UTC),
            },
        ]
        ...
        result = use_case.delete_from_backups(
            tenant_id=tenant_id, data_identifiers=["user_001", "job_001", "job_002"]
        )
        ...
        assert "daily_backup_2024_01_15" in [
            loc["backup_id"] for loc in delete_call["backup_locations"]
        ]
        assert "weekly_backup_2024_w03" in [
            loc["backup_id"] for loc in delete_call["backup_locations"]
        ]
        assert "monthly_backup_2024_01" in [
            loc["backup_id"] for loc in delete_call["backup_locations"]
        ]
...
class TestRetentionPeriodPerDataCategory:
    ...
    def test_define_retention_policies_per_category__when_system_initialized__applies_appropriate_periods(
        ...
        user_profile_result = next(
            r for r in results if r.data_category == DataCategoryEnum.USER_PROFILE.value
        )
        ...
        temp_files_result = next(
            r for r in results if r.data_category == DataCategoryEnum.TEMPORARY_FILES.value
        )
        ...
        audit_logs_result = next(
            r for r in results if r.data_category == DataCategoryEnum.AUDIT_LOGS.value
        )
...
class TestRetentionComplianceMonitoring:
    ...
    def test_check_retention_compliance__when_policies_active__reports_compliance_status(
        ...
        compliance_data = [
            {
                ...
                "next_expiry_date": datetime.now(UTC) + timedelta(days=30),
            },
            {
                "data_category": DataCategoryEnum.TEMPORARY_FILES.value,
                "total_records": 450,
                ...
                "next_expiry_date": datetime.now(UTC) + timedelta(hours=12),
            },
            {
                "data_category": DataCategoryEnum.AUDIT_LOGS.value,
                "total_records": 50000,
                ...
                "next_expiry_date": datetime.now(UTC) + timedelta(days=15),
            },
        ]
        ...
        job_data_compliance = next(
            c
            for c in result.category_compliance
