...
class TestAuditLogsAreImmutable:
    ...
    def test_create_immutable_log__when_audit_event_occurs__stores_tamper_proof_record(
        ...
        # Act
        result = use_case.execute(log_data=original_log_data, require_immutable_storage=True)
...
class TestLogsIncludeWhoWhenWhatWhy:
    ...
    def test_create_comprehensive_audit_log__when_data_subject_request__includes_all_context(
        ...
        # Act
        result = use_case.execute(
            tenant_id=tenant_id, request_id=request_id, context=comprehensive_context
        )
...
class TestNoPersonalDataInLogs:
    ...
    def test_sanitize_audit_logs__when_personal_data_present__removes_sensitive_information(
        ...
        # Act
        result = use_case.execute(
            tenant_id=tenant_id, unsafe_log_data=unsafe_log_data, require_sanitization=True
        )
...
class TestLogRetentionSeparateFromData:
    ...
    def test_audit_log_retention_policy__when_personal_data_deleted__preserves_audit_logs(
        self, mock_audit_log_repo: Mock, tenant_id: str
    ):
        """
        ...
        """
        # Arrange
        personal_data_deletion_date = datetime.now(UTC)
        audit_log_retention_years = 7  # Regulatory requirement
        expected_audit_retention_until = personal_data_deletion_date + timedelta(
            days=365 * audit_log_retention_years
        )
        ...
        use_case = QueryAuditTrailUseCase(audit_log_repo=mock_audit_log_repo)
        ...
        mock_audit_log_repo.update_retention_policy.assert_called_once_with(
            data_subject_hash="user_hash_abc123", retention_until=expected_audit_retention_until
        )
...
class TestGenerateComplianceReport:
    ...
    def test_generate_compliance_report__when_requested_by_authority__provides_comprehensive_audit_trail(
        self, mock_audit_log_repo: Mock, mock_immutable_storage_port: Mock, tenant_id: str
    ):
        """
        ...
        report_request = ComplianceReportRequest(
            tenant_id=tenant_id,
            report_period_start=datetime.now(UTC) - timedelta(days=365),
        ...
        use_case = GenerateComplianceReportUseCase(
            audit_log_repo=mock_audit_log_repo, immutable_storage_port=mock_immutable_storage_port
        )
        ...
        consent_data = next(
            c for c in compliance_data if c["event_category"] == "consent_management"
        )
        assert (
            consent_data["processing_stopped_on_withdrawal"] == consent_data["consents_withdrawn"]
        )
