...
from src.application.use_cases.submit_job import SubmitJobRequest, SubmitJobUseCase
from src.domain.job import JobStatus

from tests.fakes import FakeEventBus, FakeJobRepository, FakeRateLimiter
...
class TestSubmitJobUseCase:
    ...
    def use_case(self, job_repository, event_bus, rate_limiter):
        ...
        return SubmitJobUseCase(
            job_repository=job_repository, event_bus=event_bus, rate_limiter=rate_limiter
        )
    ...
    def test_submit_job_with_idempotency_key_first_time(
        self, use_case, submit_request, job_repository, event_bus
    ):
    ...
    def test_submit_job_with_same_idempotency_key_returns_existing(
        self, use_case, submit_request, event_bus
    ):
    ...
    def test_submit_job_sets_created_and_updated_timestamps(
        self, use_case, submit_request, job_repository
    ):
