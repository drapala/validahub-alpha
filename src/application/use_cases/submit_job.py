...
from src.application.ports import EventBus, JobRepository, LogPublisher, RateLimiter
...
class JobSubmittedEvent:
    ...
    def __post_init__(self):
        if not self.time:
            object.__setattr__(self, "time", datetime.now(UTC).isoformat())
...
class SubmitJobUseCase:
    ...
    def execute(self, request: SubmitJobRequest) -> SubmitJobResponse:
        ...
        if idempotency_key:
            ...
            if existing_job:
                ...
                self._logger.debug(
                    ...
                    tenant_id=request.tenant_id,
                    idempotency_key=(
                        request.idempotency_key if hasattr(request, "idempotency_key") else "opaque"
                    ),
        ...
        self._logger.info(
            ...
            breakdown={
                "validation_ms": (
                    validation_duration_ms if "validation_duration_ms" in locals() else 0
                ),
                "idempotency_check_ms": (
                    idempotency_duration_ms if "idempotency_duration_ms" in locals() else 0
                ),
                "rate_limit_check_ms": (
                    rate_limit_duration_ms if "rate_limit_duration_ms" in locals() else 0
                ),
                "job_creation_ms": (
                    job_creation_duration_ms if "job_creation_duration_ms" in locals() else 0
                ),
                "save_ms": save_duration_ms if "save_duration_ms" in locals() else 0,
                "event_publish_ms": (
                    event_publish_duration_ms if "event_publish_duration_ms" in locals() else 0
                ),
            },
            result="success",
            correlation_id=correlation_id,
    ...
    def _validate_request(self, request: SubmitJobRequest) -> None:
        ...
        try:
            ...
            self._logger.debug(
                "validation_failed_rules_profile_id", error=str(e), value=request.rules_profile_id
            )
            raise
