...
class Config:
    ...
    def _load_config(self) -> None:
        ...
        self.ENVIRONMENT = Environment(
            self.secrets.get("ENVIRONMENT", "development").lower()
        )
        ...
        if self.ENVIRONMENT == Environment.PRODUCTION:
            ...
            self.TRUSTED_HOSTS = [
                host.strip() for host in trusted_hosts.split(",")
                if host.strip()
            ]
        else:
            ...
            self.TRUSTED_HOSTS = [
                host.strip() for host in hosts_str.split(",")
                if host.strip()
            ]
        ...
        self.RATE_LIMIT_BURST_SIZE = int(
            self.secrets.get("RATE_LIMIT_BURST_SIZE", "10")
        )
        ...
        # Security Headers
        self.SECURITY_HEADERS_ENABLED = self.secrets.get(
        ...
        # Feature Flags
        self.FEATURE_WEBHOOKS_ENABLED = self.secrets.get(
            "FEATURE_WEBHOOKS_ENABLED", "false"
        ) == "true"
        self.FEATURE_SSE_ENABLED = self.secrets.get(
