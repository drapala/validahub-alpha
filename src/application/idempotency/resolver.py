...
def resolve_idempotency_key(
    raw_key: str | None, tenant_id: TenantId, method: str, route_template: str
) -> str:
    """
    ...
    if SECURE_KEY_PATTERN.match(raw_key) and raw_key[0] not in CSV_FORMULA_CHARS:
        logger.debug(
            "idempotency_key_accepted_as_secure", tenant_id=tenant_id.value, key_length=len(raw_key)
        )
        return raw_key
        ...
        if compat_mode == IdempotencyCompatMode.REJECT:
            security_logger.log_security_event(
                (
                    ...
                    if hasattr(SecurityEventType, "SUSPICIOUS_ACTIVITY")
    ...
    if raw_key[0] in CSV_FORMULA_CHARS:
        security_logger.injection_attempt(
            injection_type="csv_formula", field_name="idempotency_key_raw", first_char=raw_key[0]
        )
