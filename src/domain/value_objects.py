...
class TenantId:
    ...
    def __post_init__(self) -> None:
        ...
        event = ValueObjectValidationEvent.create_validation_success(
            value_object_type="TenantId", tenant_id=normalized
        )
        DomainEventCollector.collect_event(event)
...
class FileReference:
    ...
    def __post_init__(self) -> None:
        ...
        if scheme in ("http", "https"):
            ...
            if (
                not bucket
                or not _S3_BUCKET_RE.match(bucket)
                or bucket.isnumeric()
                or ".." in bucket
                or bucket.lower() != bucket
            ):
                raise ValueError("Invalid file reference")
            if not key:
                raise ValueError("Invalid file reference")
        ...
        # Emit successful validation event
        event = ValueObjectValidationEvent.create_validation_success(
            value_object_type="FileReference", file_ref=self.value, scheme=self.get_scheme()
        )
        DomainEventCollector.collect_event(event)
...
class Channel:
    ...
    def __post_init__(self) -> None:
        # Import here to avoid circular dependency
        from src.domain.events import DomainEventCollector, ValueObjectValidationEvent
